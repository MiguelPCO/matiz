...
- **Fallback general sobre la muestra estándar (10 hex × 4 tamaños × 3 dificultades × 50 seeds): 1.7%** (10/600) — el fallback (sin reposition, residuo `shiftC` de siempre) es raro ahora. Esto no equivale a "la mayoría de partidas generan una carta totalmente limpia": `grid.decidable` sigue midiendo 33 violaciones de decidibilidad sobre esa misma muestra (ver más abajo) — residuo previo a este branch, no introducido ni resuelto aquí.
...
**Sesgo posicional — DECISIÓN ABIERTA para Miguel (corrección tras whole-branch review, 2026-08-25):** el texto original de la spec describía este riesgo como "puede sesgarse hacia el centro" y lo daba por aceptado sin más. Ambas cosas eran incorrectas. Medido empíricamente sobre 12,000+ muestras: el target se sesga hacia `tr=0` (la fila más vívida, arriba), no hacia el centro — en 8×8, 32.8% de las posiciones elegidas caen en `tr=0` frente al 12.5% esperado bajo distribución uniforme (2.6x sobre-representado). La causa es estructural: en `tr=0` la restricción de croma hacia arriba es vacía (no hay filas por encima que puedan chocar con el techo de gamut), así que esa fila es la más fácil de hacer feasible.

Peor: `dificil/5×5` con `targetHex = "#e7a34b"` (el ámbar de marca del propio proyecto) tiene **una sola posición feasible** (`tr=0, tc=3`). El target cae en esa misma celda en el 100% de las partidas — 300/300 seeds probados, cero variación. No es un sesgo suave, es una posición de respuesta fija y predecible para una combinación real de tamaño/dificultad/color que el juego genera hoy. Esto queda anotado como **decisión abierta pendiente de que Miguel la vea y decida** — no como riesgo ya aceptado. `GridLattice` ahora expone `shiftL` (antes solo `shiftC`) para que este tipo de caso sea verificable en test sin depender de renderizar en vivo.
...
**Solo: pista al azar en vez de auto-puesta (branch `solo-random-clue`, 2026-08-27), pedido explícito de Miguel:** hasta ahora, Solo reutilizaba el mismo flujo de Setup que Duelo — el propio jugador escribía la palabra o subía la imagen y luego jugaba su propia ronda, viendo el swatch antes de adivinar. Tiene sentido en Duelo (pones la pista para tu rival) pero no en Solo (te pones la pista a ti mismo). Ahora, en Solo, tras elegir tamaño/dificultad/tipo de pista, la pista se genera sola vía IA (`lib/random-word.ts` → `app/api/random-word/route.ts`, nuevo endpoint, Claude con `temperature: 1` + nonce en el prompt para variar entre llamadas) y el swatch **nunca se muestra** antes de "Empezar" — mostrarlo revelaría la respuesta al mismo jugador que va a adivinar. Duelo y Diario no cambian (Duelo sigue con entrada manual, Diario es una pantalla aparte).

Tipo de pista "Imagen" queda deshabilitado en Solo (badge visual vía `Segmented`, ahora con soporte de `disabled` por opción) — no hay fuente de imágenes al azar todavía, decisión explícita de Miguel de dejarlo fuera de alcance hasta curar una galería.

Bug real encontrado en vivo (Playwright, sin `ANTHROPIC_API_KEY` en el entorno de verificación): el primer intento de la función atada a `useEffect([isSolo, draft.status])` se autocancelaba — su propio `setDraft({status:"loading"})` cambiaba `draft.status`, lo que retrigueaba el efecto, cuyo cleanup marcaba el fetch en vuelo como cancelado antes de que resolviera. La carta se quedaba colgada en "Generando pista…" para siempre. Fix: `generateRandomClue` extraída a `useCallback` sin deps, efecto solo depende de `isSolo`; "Reintentar" la llama directo en vez de resetear `draft` a `idle`.

**Solo: selector de tamaño/dificultad siempre visible (branch `solo-picker-and-diario-clue`, 2026-08-27), reportado por Miguel ("a veces al entrar por primera vez no aparece"):** el onboarding de PRD §7.1.4 (ocultar el selector en la primera partida, `state.hasPlayed`) hacía justo eso — pero `hasPlayed` solo vive en memoria (`useReducer` sin persistencia), así que cualquier recarga de página se veía como "primera partida" otra vez, ocultando el selector sin patrón aparente desde la perspectiva del usuario. Decisión explícita de Miguel: revertir esa regla — en Solo, `showPicker` (`components/screens/Setup.tsx`) ya no lee `hasPlayed`, siempre es `true`. `hasPlayed` se conserva en `lib/engine.ts`/tests (sigue siendo estado del motor correctamente probado) — solo deja de consumirse aquí.

**Diario: palabra-pista visible (mismo branch, 2026-08-27), pedido explícito de Miguel:** el color del día en Diario sigue siendo 100% determinista por fecha (`lib/daily.ts`, sin red — decisión de `2026-08-23-modo-diario-design.md` que no se toca) pero hasta ahora no tenía ninguna palabra asociada — el propio diseño original decía explícitamente "sin pista". Ahora se le pone una etiqueta al color YA generado, en la dirección inversa a Solo/Duelo (color → palabra, no palabra → color): nuevo endpoint `app/api/color-word/route.ts` + `lib/color-word.ts` (mismo patrón que `word-color`/`random-word`), llamado desde `hooks/useDaily.ts` y cacheado en `localStorage` (`matiz-daily-word-v1`, separado de la caché de resultado de partida) — un mismo jugador ve la misma palabra en cada recarga ese día. **Corrección tras code review (2026-08-27):** la caché es solo por navegador, no compartida entre jugadores — dos personas distintas pueden recibir palabras distintas para el mismo hex el mismo día, ya que cada una dispara su propia llamada la primera vez. Se puso `temperature: 0` en el endpoint como mejora best-effort (reduce varianza, no la garantiza), pero "mismos jugadores ven la misma palabra" tal cual no es una garantía real de esta implementación — quedaría pendiente una caché compartida (servidor/KV) si se necesita esa garantía. No bloquea nada: si la llamada falla, `round.clue.word` se queda en `""` y `Diario.tsx` simplemente no muestra `ClueBar`/panel "Pista" (mismo patrón "decorativo, no bloqueante" ya usado en el resto del proyecto). Verificado en vivo (Playwright): sin pista se juega igual (fallback silencioso confirmado), y con la caché de `localStorage` sembrada manualmente (sin depender de `ANTHROPIC_API_KEY` en el entorno de verificación) la pista aparece correctamente en juego y en el reveal.
